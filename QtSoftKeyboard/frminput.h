...
private:
    QString splitPinyin(QString pinyinIn);//将拼音分词
    int saveHZCZtoDB(QString ciZuHZPinyin,QString ciZuHZ);//将用户输入的词组拼音及选择的汉字词组写入数据库
...
private:
    ...
    int currentPY_count;            //当前拼音对应的词组+单汉字数量
    int currentPYCZ_count;          //当前拼音对应的汉字词组数量
    int currentHanzi_count;         //当前用户输入拼音被拆分出的汉字数量
    int currentSelect_index;        //当前用户选择的分词索引
    void selectChinese();           //查询汉字
    void showChinese();             //显示查询到的汉字
    void setChinese(int index);     //设置当前汉字
    void clearChinese();            //清空当前汉字信息

    QString m_inputPinyin;          //用于记录用户输入拼音
    QString m_selectHZ;             //用于记录用户输入拼音后选择的汉字
