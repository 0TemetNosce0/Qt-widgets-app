...
//#include "blowfish.h"
#include "pbase.h" //changing for using in a project by J.Schulze-Wenck 1/29/01
...
void CBlowFish::blowfishEncipher (DWORD *xl, DWORD *xr)
{
    union aword  Xl, Xr ;
...
void CBlowFish::blowfishDecipher (DWORD *xl, DWORD *xr)
{
    union aword  Xl ;
    union aword  Xr ;
    ...
    for (lCount = 0 ; lCount < lOutSize ; lCount += 8)
    {
        if (SameDest)   // if encoded data is being written into input buffer
        {
            ...
            else        // pad end of data with null bytes to complete encryption
            {
                ...
                blowfishEncipher((DWORD *)pInput, (DWORD *)(pInput + 4));
            }
            pInput += 8 ;
        }
        ...
        {               // input to output buffer prior to encrypting
            if (lCount < lSize - 7) // if not dealing with uneven bytes at end
            {
                ...
                // now encrypt them
                blowfishEncipher((DWORD *) pOutput, (DWORD *)(pOutput + 4));
            }
            else        // pad end of data with null bytes to complete encryption
            {
...
void CBlowFish::decode (BYTE * pInput, BYTE * pOutput, DWORD lSize)
{
    ...
    for (lCount = 0 ; lCount < lSize ; lCount += 8)
    {
        if (SameDest)   // if encoded data is being written into input buffer
        {
            blowfishDecipher ((DWORD *) pInput, (DWORD *)(pInput + 4));
            pInput += 8;
        }
        ...
        {               // so copy input to output before decoding
            ...
            blowfishDecipher((DWORD *) pOutput, (DWORD *)(pOutput + 4));
            pInput += 8;
            pOutput += 8;
        }
    }
