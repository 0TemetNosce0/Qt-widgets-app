...
AbstractWidgetList::AbstractWidgetList(QWidget *parent)
    ...
    , p(new Private)
{
    connect(&p->mapper_up, SIGNAL(mapped(QWidget*)), SLOT(upClicked(QWidget*)));
    connect(&p->mapper_down, SIGNAL(mapped(QWidget*)), SLOT(downClicked(QWidget*)));
    connect(&p->mapper_remove, SIGNAL(mapped(QWidget*)), SLOT(removeClicked(QWidget*)));
    ...
    QPushButton* addButton = new QPushButton(QIcon::fromTheme("list-add"),
                                              tr("Add New"));

    verticalLayout->addWidget(addButton);
    connect(addButton, SIGNAL(clicked()), SLOT(append()));
...
void AbstractWidgetList::appendWidget(QWidget *w)
{
    ...
    QWidget* b_up = createButton(w,&p->mapper_up,"go-up",tr("Move Up"));
    QWidget* b_down = createButton(w,&p->mapper_down,"go-down",tr("Move Down"));
    QWidget* b_remove = createButton(w,&p->mapper_remove,"list-remove",tr("Remove"));
...
QWidget *AbstractWidgetList::createButton(QWidget *data, QSignalMapper *mapper,
                                          QString icon_name, QString text, QString tooltip) const
{
...
void AbstractWidgetList::removeClicked(QWidget *w)
{
    int row = p->widgets.indexOf(w);
    remove(row);
}

void AbstractWidgetList::upClicked(QWidget *w)
{
    int row = p->widgets.indexOf(w);
    if ( row > 0 )
    {
...
void AbstractWidgetList::downClicked(QWidget *w)
{
    int row = p->widgets.indexOf(w);
    if ( row+1 < count() )
    {
