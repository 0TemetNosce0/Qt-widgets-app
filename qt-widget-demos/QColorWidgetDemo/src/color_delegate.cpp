                                      ...
                                      const QModelIndex &index) const
{
    if (index.data().canConvert<QColor>())
    {
        ...
        connect(editor, SIGNAL(colorChanged(QColor)), this, SLOT(colorChanged()));
        connect(editor, SIGNAL(accepted()), this, SLOT(closeEditor()));
        return editor;
    }
    else
    {
...
void ColorDelegate::colorChanged()
{
    ColorDialog *editor = qobject_cast<ColorDialog*>(sender());
    emit commitData(editor);
}

void ColorDelegate::closeEditor()
{
    ColorDialog *editor = qobject_cast<ColorDialog*>(sender());
